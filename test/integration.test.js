...
test('HTTP + real PostgreSQL engine: payment, delivery, authorization and administration',async t=>{
    ...
    await t.test('admin uploads documents, edits catalog and refunds a payment',async()=>{
        ...
        const leaf=(await store.hierarchy.rows()).find(node=>node.material_id==='free');
        const created=await request('/api/admin/structure',{user:999,body:{parentId:leaf.parent_id,name:'Новый вариант',requestKey:crypto.randomUUID()}});
        assert.equal(created.status,200);
        const node=created.data.node;
        const item={desc:'Задание',type:'free',telegramFileId:'admin_document_123',taskTelegramFileId:null,priceStars:null,priceRub:null,disabled:false,expectedVersion:node.version};
        assert.equal((await request('/api/admin/structure/'+node.id+'/variant',{user:999,body:item,method:'PUT'})).status,200);
        assert.equal((await request('/api/catalog')).data[node.material_id].available,true);
